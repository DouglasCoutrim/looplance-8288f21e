...
function ArenaDetailPage() {
  ...
  const isArenaAdmin = roles.some(r => r.role === 'admin_arena' && r.arena_id === id);
  const hasAccess = isSuperAdmin || isArenaAdmin;

  useEffect(() => { if (hasAccess) load(); }, [isSuperAdmin, id, roles]);

  if (loading) return <div className="flex min-h-screen items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>;
  if (!user) return <Navigate to="/login" />;
  if (!hasAccess) return <Navigate to="/acesso-negado" />;
